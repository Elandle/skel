...
if guessmethod== 0:
    ...
    guess= guesst[:N//2]+ guessv[:(N+ 1)//2]+ [guesstime]+ [e]
if guessmethod== 1:
    #       t   v   time    u
    guess= [                 ]
...
if boundsmethod== 0:
    #                            t                                  v                       time      e
    bounds= tuple([(0, 5) for i in range(N//2)]+ [(0.1, 5) for i in range((N+ 1)//2)]+ [(0, 100)]+ [(0, 5)])
if boundsmethod== 1:
    bounds= tuple([(0, 100) for i in range(N//2)]+ [(0, 100) for i in range((N+ 1)//2)]+ [(0, 100)]+ [(0, 100)])
...
filedata= open("PamDualAnnealingFixedulowData.txt", "a")
fileminima= open("PamDualAnnealingFixedulowMinima.txt", "a")
...
def printminimaeven(x, f, context):
    ...
    addminima= f"{normalizationdata} {model} {1-f} {x[-1]} {elow} {x[-2]} {N} {u} {d} {startingstring} {endingstring} {tstring} {vstring}\n"
...
if N%2== 0:
    for i in range(iterations):
        ...
        vstring= ",".join(map(str, list(v)))
        #               fixedu           Pam      fidel    u   ulow   time   N   u   d   starting state   ending state      t      v
        adddata= f"{normalizationdata} {model} {fidelity} {x[-1]} {elow} {x[-2]} {N} {u} {d} {startingstring} {endingstring} {tstring} {vstring}\n"

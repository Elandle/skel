...
if guessmethod== 0:
    ...
    guess= guesst[:N//2]+ guessv[:(N+ 1)//2]+ [guesse]+ [guesstime]
if guessmethod== 1:
    #       t   v   e   time
    guess= [                ] 
...
if boundsmethod== 0:
    #                            t                               v                        e          time
    bounds= tuple([(0, 20) for i in range(N//2)]+ [(1, 20) for i in range((N+ 1)//2)]+ [(1, 20)]+ [(0, 60)])
if boundsmethod== 1:
...
filedata= open("pamdaFixedulowData.txt", "a")
fileminima= open("pamdaFixedulowMinima.txt", "a")
...
# x=[t[0], t[1], t[2], ..., t[N//2], v[0], v[1], ..., e, time]
def minfideven(x):

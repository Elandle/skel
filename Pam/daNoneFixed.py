...
if guessmethod== 0:
    ...
    guesse= float(guessrun[3])
    guesselow= float(guessrun[4])
    ...
    guess= guesst[:N//2]+ guessv[:(N+ 1)//2]+ [guesse]+ [guesselow]+ [guesstime]
if guessmethod== 1:
    #       t   v   e   elow   time
    guess= [                       ] 
...
if boundsmethod== 0:
    #                            t                               v                         e         elow       time
    bounds= tuple([(0, 20) for i in range(N//2)]+ [(1, 20) for i in range((N+ 1)//2)]+ [(0, 10)]+ [(1, 20)]+ [(0, 60)])
if boundsmethod== 1:
    bounds= tuple([(0, 100) for i in range(N//2)]+ [(0, 100) for i in range((N+ 1)//2)]+ [(0, 100)]+ [(0, 100)]+ [(0, 100)])
...
model= "Pam"
normalizationdata= "nonefixed"

# Files to append results to; note data is appended each run and does not make a new file each run
filedata= open("pamdaNoneFixedData.txt", "a")
fileminima= open("pamdaNoneFixedMinima.txt", "a")
...
# x=[t[0], t[1], t[2], ..., t[N//2], v[0], v[1], ..., e, elow, time]
def minfideven(x):

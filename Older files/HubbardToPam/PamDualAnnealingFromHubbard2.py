...
run= "fixedu Hubbard 0.9919658590176814 10.5 46.22857454883429 6 1 1 0,0 5,5 0.6967266782457019,1.957537972606599,1.2837463523291812,1.957537972606599,0.6967266782457019"
run= run.split()
e= float(run[3])
...
#                 vc         tc        elow       time
bounds= tuple([(0.1, 10)]+ [(0, 20)]+ [(0, 120)]+ [(0, 75)])
...
model= "Pam"
normalizationdata= "fromhubbardvc2"
...
filedata= open("PamDualAnnealingFromHubbardvc2.txt", "a")
fileminima= open("PamDualAnnealingFromHubbardvc2.txt", "a")
...
def minfid(x):
    ...
    elow= x[2]
    time= x[3]
    t= [tc*a for a in thubbard]
    v= [vc for i in range(N)]
